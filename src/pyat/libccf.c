...
/* c function to calculate ccf and determine peak */
...
{
  int i, j, jbeg, jend, nintp, nmax, idx_max_temp;
  double dtau, taui, mean1, mean2, std1, std2, std12, ccf12, w12, ccf21, w21;
  double ccf_max, tau_temp;
  double *fintp;
...
/* c function to calculate ccf, determine peak and centroid */
...
{
  int i, j, jbeg, jend, nabove, idx_max;
  double ccf_max, tau_temp, norm_temp;
  int *idx_above;

  /* calculate ccf and peak */
  ciccf_peak_proto(t1, f1, n1, t2, f2, n2, ntau, tau_beg, tau_end, ways,
                   tau, ccf, rmax, &idx_max, tau_peak);
...
{
  /* ways = 0: two-way
   * ways = 1: interpolation on 1st 
   * ways = 2: interpolation on 2nd
   */
  ciccf_proto(t1, f1, n1, t2, f2, n2, ntau, tau_beg, tau_end,
              threshold, mode, ignore_warning, 0, 
    ...
    double tau[], double ccf[], double *rmax, double *tau_peak, double *tau_cent)
{
   ...
   */
  ciccf_proto(t1, f1, n1, t2, f2, n2, ntau, tau_beg, tau_end,
              threshold, mode, ignore_warning, 1, 
              tau, ccf, rmax, tau_peak, tau_cent);
  return;
}
    ...
    double tau[], double ccf[], double *rmax, double *tau_peak)
{
   ...
   * ways = 2: interpolation on 2nd
   */
  ciccf_peak_proto(t1, f1, n1, t2, f2, n2, ntau, tau_beg, tau_end, 0,
                   tau, ccf, rmax, &idx_max, tau_peak);
  return;
}
    ...
    double tau[], double ccf[], double *rmax, double *tau_peak)
{
   ...
   * ways = 2: interpolation on 2nd
   */
  ciccf_peak_proto(t1, f1, n1, t2, f2, n2, ntau, tau_beg, tau_end, 1,
                   tau, ccf, rmax, &idx_max, tau_peak);
  return;
}
             ...
             )
{
  ...
  for(i=0; i<nsim; i++)
  {
    ...
    ciccf_proto(t1_sim, f1_sim, n1_sim, t2_sim, f2_sim, n2_sim,
          ntau, tau_beg, tau_end, threshold, mode, ignore_warning, ways,
          tau, ccf, &rmax, &tau_peak, &tau_cent
         );
             ...
             )
{
   ...
   * ways = 2: interpolation on 2nd
   */
  ciccf_mc_proto(t1, f1, e1, n1, t2, f2, e2, n2, ntau, tau_beg, tau_end, nsim,
                 threshold, mode, ignore_warning, 0, 
